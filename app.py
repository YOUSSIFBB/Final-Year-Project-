...
#URL scanner
# PDF Class for URL Scan
class URLPDF(FPDF):
    def header(self):
        self.image('static/logo-color.png', x=10, y=8, w=50)
    ...
    def engines_table(self, engines):
        ...
        self.cell(40, 7, "Category", 1, 0, 'C', True)
        self.cell(0, 7, "Result", 1, 1, 'C', True)

        self.set_font('Arial', '', 11)
        for engine, data in engines.items():
            self.cell(60, 6, engine, 1)
            self.cell(40, 6, data.get('category', ''), 1)
            self.cell(0, 6, data.get('result', ''), 1, 1)
        self.ln(4)
...
def download_url_report():
    ...
    pdf.section_title("Top Engine Results")
    pdf.engines_table(engines)

    now = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    filename = f"url_report_{now}.pdf"
