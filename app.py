...
class URLPDF(FPDF):
    def header(self):
        self.image('static/logo-color.png', x=10, y=8, w=60)
        self.set_font('Arial', 'B', 18)
        self.cell(0, 10, 'ThreatGuard - URL Scan Report', ln=True, align='C')
        self.ln(10)
    ...
    def section_title(self, txt):
        ...
        self.set_text_color(255)
        self.set_font('Arial', 'B', 14)
        self.cell(0, 8, txt, 0, 1, 'L', fill=True)
        self.ln(2)

    def key_value_table(self, mapping):
        self.set_font('Arial', '', 12)
        for k, v in mapping.items():
            self.set_fill_color(220,220,220)
            self.cell(50, 7, str(k), 1, 0, 'L', True)
            self.cell(0, 7, str(v), 1, 1, 'L')
        self.ln(4)

    def engines_table(self, engines):
        # header
        self.set_font('Arial', 'B', 12)
        self.set_fill_color(200,200,200)
        self.cell(60, 7, "Engine", 1, 0, 'C', True)
        self.cell(30, 7, "Category", 1, 0, 'C', True)
        self.cell(30, 7, "Method", 1, 0, 'C', True)
        self.cell(60, 7, "Result", 1, 1, 'C', True)
        # rows
        self.set_font('Arial', '', 11)
        for engine, data in engines.items():
            self.cell(60, 6, engine, 1)
            self.cell(30, 6, data.get('category',''), 1)
            self.cell(30, 6, data.get('method',''), 1)
            self.cell(60, 6, data.get('result',''), 1, 1)
        self.ln(4)
...
def url_scan():
    ...
    if request.method == 'POST':
        ...
        if r.status_code == 200:
            ...
            # 2) poll
            for _ in range(15):
                analysis = requests.get(f'https://www.virustotal.com/api/v3/analyses/{scan_id}', headers=headers).json()
            ...
            flash('Failed to submit URL', 'error')
    ...
    return render_template('urlScan.html',
                           verdict=verdict,
                           report_json=json.dumps(report, indent=2) if report else None,
                           report=report)

@app.route('/download-url-report', methods=['POST'])
def download_url_report():
    # pulls hidden form fields
    url = request.form['scanned_url']
    verdict = request.form['verdict']
    report = json.loads(request.form['report_json'])

    # header info
    attrs = report['data']['attributes']
    stats = attrs['stats']
    date = datetime.datetime.utcfromtimestamp(attrs['date']).strftime('%Y-%m-%d %H:%M:%S UTC')
    ...
    pdf.section_title("Per-Engine Results")
    engines = attrs['results']
    pdf.engines_table(engines)

    # unique filename
    now = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    fname = f"url_report_{now}.pdf"
